...
kind: "mod"
emoji: "🧰"
summary: "Version 0.1.3 hardens ModForge against two security issues: a prototype pollution flaw in the patch composer's field-op paths and a path-traversal weakness in the development preview server's directory containment check, both of which now reject malformed input properly. It also adds a repository-specific SECURITY.md describing which surfaces (draft persistence, path validation, ZIP construction, DOM rendering, session-test refusal, preview server) fall under this repo's scope."
---
