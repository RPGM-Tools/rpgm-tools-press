...
| Platform | File |
| --- | --- |
| Windows (installer) | `Neo Angband Setup 1.3.0.exe` |
| Windows (portable, one file) | `Neo Angband-1.3.0-portable.exe` |
| macOS | `.dmg`, or the `.zip` if you prefer to unpack it yourself |
| Linux | `.AppImage` (no install), `.deb`, or `.tar.gz` |
| Self-hosting | `neo-angband-web-1.3.0.zip` - static files, any web server |
